...
Page({
  ...
  markFollowing() {
    ...
    const task = this.data.type === "demand"
      ? demandService.updateDemandStatusToCloud(this.data.id, app.globalData.activeUserId, "following")
      : service.updateStatusToCloud(this.data.id, app.globalData.activeUserId, INVENTORY_STATUS.FOLLOWING, "标记跟进中");
    task
      .then(() => {
        this.loadData();
        wx.showToast({ title: "已标记跟进中", icon: "success" });
      })
      .catch((error) => wx.showToast({ title: error.message, icon: "none" }));
  },
  ...
  markSold() {
    ...
    const isAdmin = authService.isAdminUser(currentUser);
    wx.showModal({
      title: isAdmin ? "确认完成" : "提交完成审核",
      content: isAdmin
        ? (this.data.type === "demand" ? "确认后该需求将直接标记为已完成并下线。" : "确认后该货源将直接标记为已完成并下架。")
        : (this.data.type === "demand" ? "需求完成后需对应组管理员审核，通过后将从需求池下架，并在我的页面留存。" : "货源完成后需对应组管理员审核，通过后将从货源池下架，并在我的页面留存。"),
      success: (res) => {
        if (!res.confirm) {
          return;
        }
        const task = this.data.type === "demand"
          ...
          : service.updateStatusToCloud(this.data.id, app.globalData.activeUserId, INVENTORY_STATUS.SOLD, "标记已完成");
        task
          .then(() => {
            this.loadData();
            wx.showToast({ title: isAdmin ? "已完成" : "已提交审核", icon: "success" });
          })
          .catch((error) => wx.showToast({ title: error.message, icon: "none" }));
      }
    });
      ...
      success: ({ tapIndex }) => {
        if (tapIndex === 0) {
          const task = isDemand
            ...
            : service.directOfflineItemToCloud(this.data.id, app.globalData.activeUserId, "货源已在别处售出");
          task.then(() => {
            this.loadData();
          ...
          success: (res) => {
            ...
            const task = isAdmin
              ...
              : (isDemand
                ...
                : service.requestOfflineItemToCloud(this.data.id, app.globalData.activeUserId, reason));
            task.then(() => {
              this.loadData();
              wx.showToast({ title: isAdmin ? "已下架" : "已提交审核", icon: "success" });
            }).catch((error) => wx.showToast({ title: error.message, icon: "none" }));
          }
        });
      }
  ...
  cancelFollow() {
    ...
    const isAdmin = authService.isAdminUser(currentUser);
    const isOwnFollow = item.followOwnerId === app.globalData.activeUserId;
    if (!isOwnFollow && !isAdmin) {
      wx.showToast({ title: "只能取消自己的跟进", icon: "none" });
      return;
    }
    const task = this.data.type === "demand"
      ? demandService.cancelFollowDemandToCloud(this.data.id, app.globalData.activeUserId)
      : service.cancelFollowItemToCloud(this.data.id, app.globalData.activeUserId);
    task
      .then(() => {
        this.loadData();
        wx.showToast({ title: "已取消跟进", icon: "success" });
      })
      .catch((error) => wx.showToast({ title: error.message, icon: "none" }));
  },
