...
const BASE_URL = "https://21068849.xyz/api/mini/items";
const UPLOAD_IMAGE_URL = "https://21068849.xyz/api/mini/upload/image";
const UPLOAD_VIDEO_URL = "https://21068849.xyz/api/mini/upload/video";
const BOOTSTRAP_URL = "https://21068849.xyz/api/mini/bootstrap";
const ORIGIN_URL = "https://21068849.xyz";
const TOKEN = "kDHAM378zqHTh3TSve5Na0LL3HW3vqecBOoqSvduOxw";
...
function request({ url = BASE_URL, method = "GET", data = null }) {
  return new Promise((resolve, reject) => {
    wx.request({
      ...
      fail: (error) => reject(new Error(error.errMsg || "云端接口请求失败"))
    });
  });
}
...
function mapRemoteSupply(item) {
  ...
  const ownerInfo = item.ownerInfo || extras.ownerInfo || "";
  ...
  return {
    ...
    nic: machineConfig.nic1 || machineConfig.nic2 || "",
    specDetails: {
      brand: machineConfig.brand || "",
      model: machineConfig.model || item.title || "",
      cpu: machineConfig.cpu || "",
      ...
      gpu: machineConfig.gpu || "",
      nic: [machineConfig.nic1, machineConfig.nic2].filter(Boolean).join(" / "),
...
function mapRemoteDemand(item) {
  ...
  return {
    ...
    sourceType: item.scope === "company" ? "company" : "own",
    imageUrls,
    mediaFiles,
    creatorId: owner ? owner.id : `remote_${creatorName}`,
    creatorName,
...
function toRemoteDemand(item, activeUserId) {
  ...
  return {
    ...
    videoUrls: getRemoteVideoUrls(item),
    mediaFiles: getRemoteMediaFiles(item),
    scope: item.sourceType === "own" ? "mine" : "company",
    ownerName: owner.name,
    followOwnerId: item.followOwnerId || "",
    followOwnerName: item.followOwnerName || "",
    lastFollowedAt: item.lastFollowedAt || "",
...
function fetchItems(side) {
  return request({ url: `${BASE_URL}?side=${encodeURIComponent(side)}&includeDeleted=1` })
...
function deleteItem(id) {
  return request({ url: `${BASE_URL}/${encodeURIComponent(id)}`, method: "DELETE" });
}
...
function bootstrapFromMini(supplies = [], demands = [], activeUserId = "") {
  const supplyPayloads = supplies
    .filter((item) => item && !item.isDeleted)
    .map((item) => toRemoteSupply(item, activeUserId));
  const demandPayloads = demands
    .filter((item) => item && !item.isDeleted)
    .map((item) => toRemoteDemand(item, activeUserId));
  return request({
    url: BOOTSTRAP_URL,
    method: "POST",
    data: {
      confirm: "mini-program-source-of-truth",
      items: supplyPayloads.concat(demandPayloads)
    }
  });
}
