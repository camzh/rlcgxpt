...
const SUPPLY_ALIASES = {
  ...
  nic3: ["网卡3"],
  nic4: ["网卡4"],
  extraNics: ["更多网卡", "其他网卡", "多网卡"],
  raid: ["Raid卡", "RAID", "阵列卡"],
  powerSupply: ["电源", "PSU"],
  pcieSwitch: ["PCIE交换芯片", "PCIe交换芯片", "PCIE芯片", "PCIe芯片", "交换芯片"],
  saleUnit: ["售卖单位", "出售单位"],
  rentalUnit: ["租赁单位"],
  oneYearFull: ["一年全包"],
  twoYearFull: ["两年全包"],
...
const TYPE_ALIASES = ["类型", "side", "导入类型"];
...
function applyLooseConfig(row, aliases) {
  ...
  if (!row.pcieSwitch) row.pcieSwitch = pickMatch(text, [/(?:PCIE交换芯片|PCIe交换芯片|PCIE芯片|PCIe芯片|交换芯片)[:：\s]*([^,，;；\/]{2,30})/i]);
...
function normalizeSupply(row) {
  ...
  const rentalPrices = {
    oneYearFull: row.oneYearFull || "",
    twoYearFull: row.twoYearFull || "",
    threeYearFull: row.threeYearFull || "",
    oneYearMove: row.oneYearMove || "",
    twoYearMove: row.twoYearMove || "",
    threeYearMove: row.threeYearMove || ""
  };
  const hasRentalPrice = Object.values(rentalPrices).some((value) => String(value || "").trim());
  return {
    ...DEFAULT_SUPPLY,
    ...row,
    title,
    ...
    businessType: hasRentalPrice && row.price ? "both" : hasRentalPrice ? "rent" : "sale",
    rentalPrices,
    priceUnit: row.saleUnit || DEFAULT_SUPPLY.priceUnit,
    ...
    specDetails: {
      ...
      cpu: row.cpu || "",
      memory: row.memory || "",
      storage: row.dataDisk || row.storage || "",
      m2: row.systemDisk || "",
      gpu: row.gpu || "",
      nic: row.nic || "",
      nic2: row.nic2 || "",
      nic3: row.nic3 || "",
      nic4: row.nic4 || "",
      extraNics: row.extraNics || "",
      raid: row.raid || "",
      powerSupply: row.powerSupply || "",
      pcieSwitch: row.pcieSwitch || ""
    }
  };
}
...
function buildItems(type, rows) {
  ...
  const typeIndex = (rows[0] || []).findIndex((header) => TYPE_ALIASES.includes(String(header || "").trim()));
  const hasHeaders = Object.keys(headerMap).length > 0;
  const dataRows = hasHeaders ? rows.slice(1) : rows;
  return dataRows.filter((cells) => {
    if (!hasHeaders || typeIndex < 0) return true;
    const rawType = String(cells[typeIndex] || "").trim().toLowerCase();
    if (!rawType) return true;
    return type === "supply" ? ["supply", "货源", "供应"].includes(rawType) : ["demand", "需求", "求购"].includes(rawType);
  }).map((cells, index) => {
